	...
	public static partial class PromptManager
	{
		public static string GetSystemPrompt(string subjectArea, List<string>? sources)
		{
			...
			var systemPrompt =
				...
				$"\n- Если вопрос общий, нечеткий или содержит несколько тем → не отвечайте сразу, а ОБЯЗАТЕЛЬНО ЗАПРОСИТЕ УТОЧНЕНИЕ." +
		...
		public static string GetSmallTalkPrompt(string subjectArea) =>
			...
			"Если вы получаете вопрос по твоей теме, в которой ты эксперт, то ВАМ НАДО ОБРАТИТЬСЯ К ВНУТРЕННЕЙ БАЗЕ ЗНАНИЙ, ничего не отвечайте, а верните оригинальное сообщение пользователя в формате:\n" +
			"RESPONSE: <оригинальное сообщение пользователя>\n" +
			"Это позволит получить правильный ответ и сделает пользователя счастливым!!!\n" +
			"Примеры:\n" +
			...
			"Пользователь: Сколько весил трицератопс?\nАссистент: RESPONSE: Сколько весил трицератопс?\n" +
			"Пользователь: В какой период жил алиорам?\nАссистент: RESPONSE: В какой период жил алиорам?\n" +
