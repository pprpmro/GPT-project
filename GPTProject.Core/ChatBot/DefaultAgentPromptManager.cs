	...
	public static partial class PromptManager
	{
		public static string GetSystemPrompt(string subjectArea, List<string>? sources)
		{
			...
			var systemPrompt =
				...
				$"Отвечайте кратко, СТРОГО СЛЕДУЙ БАЗЕ ЗНАНИЙ, не добавляйте предположений." +
				$"Не бери ничего из своих знаний, только то что в базе знаний" +
				...
				$"\n- Если вопрос общий, нечеткий или содержит несколько тем → не отвечайте сразу, а запросите уточнение." +
				$"\n- Если если в базе знаний отсутствует необходимая информация → скажите, скажите пользователю что не знаете" +
				$"\n- Если вопрос не по теме,  сообщите об этом пользователю." +
				...
				$"RESPONSE: <краткий ответ> или CLARIFICATION_QUESTION: <уточняющий вопрос>\n" +

				$"\nПримеры:\n" +
				$"RESPONSE: API-интерфейс служит для взаимодействия с моделью.\n" +
				$"CLARIFICATION_QUESTION: Уточните, какую версию ПО вы используете?\n" +
				$"RESPONSE: Информация о таком функционале в базе знаний отсутствует.\n" +
				$"CLARIFICATION_QUESTION: Какой именно алгоритм вас интересует?\n" +
		...
		public static string GetQuestionSeparatingPrompt() =>
			...
			"\r\n- Запросы – это фразы, которые не сформулированы как вопросы, но требуют информации (например, \"Расскажи об этом\", \"Опиши это\")." +
		...
		public static string GetSmallTalkPrompt(string subjectArea) =>
			...
			"Отвечайте пользователю на все small talk запросы (шутки, приветствия, личные вопросы и т.п.), а также на простые или общие вопросы, если текущего контекста достаточно для ответа.\n" +
			"Отвечай кратко и по делу, не пиши длинные ответы. Максимально четко." +
			"Если вы сомневаетесь, является ли вопрос общим или специализированным — всегда пересылайте сообщение дальше, не отвечая самостоятельно.\n" +
			"Если вы получаете серьёзный или специализированный вопрос, ответ на который требует обращения к базе знаний, ничего не отвечайте, а верните оригинальное сообщение пользователя в формате:\n" +
			"RESPONSE: <оригинальное сообщение пользователя>\n" +
			"Примеры:\n" +
			"Пользователь: Привет, как дела?\nАссистент: Привет! Всё отлично!\n" +
			"Пользователь: Расскажи анекдот.\nАссистент: Почему программисты путают Хэллоуин и Рождество? Потому что OCT 31 == DEC 25.\n" +
			"Пользователь: Что такое API?\nАссистент: RESPONSE: Что такое API?\n" +
			"Пользователь: Какое максимальное число токенов у GPT-4?\nАссистент: RESPONSE: Какое максимальное число токенов у GPT-4?\n" +
			"Пользователь: Чем занимаешься?\nАссистент: Помогаю пользователям отвечать на вопросы и общаюсь с тобой!";
	}
}